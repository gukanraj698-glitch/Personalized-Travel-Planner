    ...
    try:
        with db.get_db() as conn:
            with conn.cursor() as cur:
                if not uid:
                    # Auto-fallback to default demo traveller for immediate zero-friction exploration
                    cur.execute("SELECT id, full_name, email, role, phone, loyalty_points, tier, avatar_url, created_at FROM users WHERE lower(email)='traveller@wanderly.com'")
                    user = cur.fetchone()
                    if user:
