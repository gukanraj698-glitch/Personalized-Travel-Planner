...
async function openProfileModal() {
  ...
  if (!res || !res.success) {
    showToast("Unable to load profile. Please sign in.");
    return;
  }
  const u = res.user;
  const avatar = u.avatar_url || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80";
  ...
  const modalHtml = `
    <div style="max-width: 580px; margin: 0 auto;">
      <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 20px; border-bottom: 1px solid var(--border-color); padding-bottom: 16px;">
        <img src="${avatar}" style="width: 72px; height: 72px; border-radius: 50%; object-fit: cover; border: 3px solid var(--accent);" alt="User Avatar">
        <div style="flex: 1;">
          <div style="display: flex; align-items: center; gap: 8px;">
            <h2 style="margin: 0; font-size: 1.4rem;">${u.full_name}</h2>
            <span style="background: var(--accent-light); color: var(--accent-dark); padding: 2px 10px; border-radius: 20px; font-size: 11px; font-weight: 800; border: 1px solid var(--accent);">${u.tier} Member</span>
          </div>
          <p style="margin: 4px 0 0; color: var(--text-muted); font-size: 0.85rem;">${u.email} · Role: <b>${u.role.toUpperCase()}</b></p>
        </div>
      </div>

      <!-- LOYALTY POINTS BADGE -->
      <div style="background: linear-gradient(135deg, rgba(22,163,74,0.08), rgba(20,83,45,0.12)); border: 1px solid rgba(22,163,74,0.3); border-radius: 12px; padding: 16px; margin-bottom: 20px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          <span style="font-weight: 700; font-size: 0.9rem;">💎 Wanderly Privilege Club</span>
          <b style="color: #16a34a; font-size: 1.1rem;">${u.loyalty_points.toLocaleString()} Points</b>
        </div>
        <p style="margin: 0; font-size: 0.78rem; color: var(--text-muted);">Redeemable value: <b>₹${Math.round(u.loyalty_points * 0.5).toLocaleString()}</b> on holiday bookings & upgrades.</p>
      </div>

      <!-- QUICK STATS -->
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-bottom: 20px;">
        <div style="background: var(--bg-surface); border: 1px solid var(--border-color); padding: 12px; border-radius: 10px; text-align: center;">
          <small style="color: var(--text-muted); display: block; font-size: 11px;">Tier Level</small>
          <b style="font-size: 1rem; color: var(--text-primary);">${u.tier}</b>
        </div>
        <div style="background: var(--bg-surface); border: 1px solid var(--border-color); padding: 12px; border-radius: 10px; text-align: center; cursor: pointer;" onclick="closeModal(); switchTab('trips');">
          <small style="color: var(--text-muted); display: block; font-size: 11px;">My Bookings</small>
          <b style="font-size: 1rem; color: var(--accent);">View Trips →</b>
        </div>
        <div style="background: var(--bg-surface); border: 1px solid var(--border-color); padding: 12px; border-radius: 10px; text-align: center; cursor: pointer;" onclick="closeModal(); switchTab('wishlist');">
          <small style="color: var(--text-muted); display: block; font-size: 11px;">Saved Items</small>
          <b style="font-size: 1rem; color: var(--accent);">Wishlist →</b>
        </div>
      </div>

      <!-- EDIT PROFILE FORM -->
      <h3 style="font-size: 1rem; margin-bottom: 12px;">Edit Personal Details</h3>
      <form onsubmit="saveProfileChanges(event)" style="display: flex; flex-direction: column; gap: 12px;">
        <div class="form-group" style="margin: 0;">
          <label style="font-size: 11px; font-weight: 700; color: var(--text-muted);">Full Name</label>
          <input type="text" name="full_name" value="${u.full_name}" required class="form-control">
        </div>
        <div class="form-group" style="margin: 0;">
          <label style="font-size: 11px; font-weight: 700; color: var(--text-muted);">Email Address</label>
          <input type="email" value="${u.email}" disabled class="form-control" style="opacity: 0.6; cursor: not-allowed;">
        </div>
        <div class="form-group" style="margin: 0;">
          <label style="font-size: 11px; font-weight: 700; color: var(--text-muted);">Phone Number</label>
          <input type="tel" name="phone" value="${u.phone || ''}" placeholder="+91 98765 43210" class="form-control">
        </div>
        <div style="display: flex; gap: 10px; margin-top: 10px;">
          <button type="submit" class="btn-primary" style="flex: 1;">Save Profile Changes</button>
          <button type="button" onclick="closeModal()" class="btn-secondary" style="padding: 0 16px;">Close</button>
        </div>
      </form>
    </div>
  `;
